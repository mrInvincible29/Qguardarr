...
class DockerManager:
    ...
    def cleanup_containers(self) -> bool:
        ...
        try:
            ...
            subprocess.run(
                ["docker", "container", "prune", "-f"], capture_output=True, timeout=10
            )

            # Clean up volumes
            subprocess.run(
                ["docker", "volume", "prune", "-f"], capture_output=True, timeout=10
            )
    ...
    async def wait_for_service(
        self, service_name: str, url: str, timeout: int = 60
    ) -> bool:
...
class QBittorrentHelper:
    ...
    def __init__(
        self, host="localhost", port=8080, username="admin", password="adminadmin"
    ):
        self.host = host
        self.port = port
                    ...
                    try:
                        ...
                        if response.text.strip() == "Ok.":
                            ...
                            response = await client.get(
                                f"{self.base_url}/api/v2/app/version", timeout=5.0
                            )
    ...
    async def get_test_torrent_info(self) -> List[Dict]:
        ...
        try:
            response = await client.get(
                f"{self.base_url}/api/v2/torrents/info", timeout=10.0
            )
...
class QguardarrHelper:
    ...
    async def send_test_webhook(self, event_data: Dict) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.base_url}/webhook", data=event_data, timeout=5.0
                )
...
def skip_if_no_docker(func):
    ...
    return pytest.mark.skipif(
        not docker_mgr.is_docker_available(), reason="Docker not available"
    )(func)
...
def skip_if_no_compose(func):
    ...
    docker_mgr = DockerManager()
    return pytest.mark.skipif(
        not docker_mgr.is_compose_available(), reason="Docker Compose not available"
    )(func)
