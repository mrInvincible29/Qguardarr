...
from src.webhook_handler import WebhookHandler, WebhookEvent
...
class TestSLACompliance:
    ...
    async def test_new_torrent_gets_limit_within_2_minutes(self, integration_config):
        ...
        mock_qbit_client.needs_update = Mock(return_value=True)  # Sync mock for sync method
    ...
    async def test_webhook_response_time_sla(self, integration_config):
        ...
        webhook_handler = WebhookHandler(
            config=integration_config,
            allocation_engine=mock_allocation_engine
        )
    ...
    async def test_allocation_cycle_performance_sla(self, integration_config):
        ...
        print(f"✅ Allocation cycle for {len(test_torrents)} torrents completed in {cycle_time:.3f}s (SLA: <10s)")
