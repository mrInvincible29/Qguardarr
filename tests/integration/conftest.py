...
async def docker_services(docker_manager):
    ...
    try:
        ...
        if not await docker_manager.wait_for_service("qBittorrent", "http://localhost:8080", timeout=90):
        ...
        qguardarr_ready = await docker_manager.wait_for_service("Qguardarr", "http://localhost:8089/health", timeout=60)
...
def public_torrents_data():
    ...
    torrents_file = Path(__file__).parent.parent.parent / "test-data" / "public_torrents.json"
...
def integration_config(tmp_path):
    ...
    return QguardarrConfig(**{
        ...
        "rollback": RollbackSettings(
            database_path=str(tmp_path / "test_rollback.db"),
...
def assert_torrent_limit_within_range(actual: int, expected: int, tolerance: float = 0.01):
    ...
    if expected == -1:  # Unlimited
        ...
        assert min_expected <= actual <= max_expected, f"Expected {expected}, got {actual} (tolerance: {tolerance})"
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "qbittorrent: mark test as requiring qBittorrent")
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        if "integration" in str(item.fspath):
            ...
            if any(keyword in item.name for keyword in ["end_to_end", "performance", "load"]):
