...
from src.config import QguardarrConfig, RollbackSettings, GlobalSettings, QBittorrentSettings, TrackerConfig, CrossSeedSettings, LoggingSettings
...
class TestRollbackIntegration:
    ...
    async def test_basic_rollback_functionality(self, rollback_manager):
        ...
        assert changes_recorded == len(changes), f"Expected {len(changes)} changes, recorded {changes_recorded}"
        
        # Verify changes were recorded using stats
        stats = await rollback_manager.get_rollback_stats()
        assert stats["total_entries"] >= len(changes), "Should have recorded all changes"
        ...
        for hash_, old_limit, new_limit, tracker, reason in changes:
            ...
            assert rollback_data[hash_] == old_limit, f"Original limit should be {old_limit}"
        ...
        print(f"✅ Successfully recorded {changes_recorded} changes and prepared rollback for {rollback_count} torrents")
    ...
    async def test_rollback_performance(self, rollback_manager):
        ...
        assert record_time < 1.0, f"Recording 100 changes took {record_time:.3f}s, should be <1s"
        ...
        assert retrieval_time < 1.0, f"Retrieving rollback data took {retrieval_time:.3f}s, should be <1s"
        
        print(f"✅ Performance test: Record {record_time:.3f}s, Retrieve {retrieval_time:.3f}s for 100 changes")