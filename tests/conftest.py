    ...
    return QguardarrConfig(
        **{
            ...
            "rollback": RollbackSettings(
                database_path=str(temp_dir / "test_rollback.db"), track_all_changes=True
            ),
            "cross_seed": CrossSeedSettings(
                enabled=False, url=None, api_key=None, timeout=15
            ),
            "logging": LoggingSettings(
                level="INFO", file="./logs/test.log", max_size_mb=10, backup_count=3
            ),
        }
    )
...
def mock_rollback_manager():
    ...
    manager.get_stats.return_value = {"changes_recorded": 0, "rollbacks_performed": 0}
...
def assert_torrent_limit_within_range(
    actual_limit: int, expected_limit: int, tolerance: float = 0.1
):
...
def assert_response_time_acceptable(response_time_ms: float, max_ms: float = 100):
    ...
    ), f"Response time {response_time_ms:.1f} ms exceeds limit of {max_ms} ms"
