...
class TorrentGenerator:
    ...
    def generate_torrent(self, index: int) -> TorrentInfo:
        ...
        return TorrentInfo(
            ...
            - (index % 3600),  # Activity within last hour
        )
...
class PerformanceMonitor:
    ...
    def start_monitoring(self):
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                if "python" in proc.info["name"] and "src.main" in str(
                    proc.info["cmdline"]
                ):
    ...
    def take_measurement(self, label: str) -> Dict[str, Any]:
        ...
        if self.process:
            try:
                measurement.update(
                    {
                        "process_memory_mb": self.process.memory_info().rss
                        / 1024
                        / 1024,
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        process_memory = [
            m.get("process_memory_mb", 0)
            for m in self.measurements
            if "process_memory_mb" in m
        ]
        process_cpu = [
            m.get("process_cpu_percent", 0)
            for m in self.measurements
        ...
        return {
            ...
            "process_memory_mb": {
                ...
                "avg": (
                    sum(process_memory) / len(process_memory) if process_memory else 0
                ),
            },
            "process_cpu_percent": {
...
class TestMemoryUsage:
    ...
    async def test_memory_usage_500_torrents(self, monitor, generator):
        ...
        from src.allocation import AllocationEngine, TorrentCache
        from src.config import GlobalSettings, QguardarrConfig
        ...
        for i, torrent in enumerate(torrents):
            cache.add_torrent(
                torrent.hash, f"tracker{i % 5}", torrent.upspeed, 1024000 + (i * 1000)
            )
    ...
    async def test_memory_usage_3000_torrents(self, monitor, generator):
        ...
        for i, torrent in enumerate(torrents):
            success = cache.add_torrent(
                torrent.hash, f"tracker{i % 10}", torrent.upspeed, 1024000 + (i * 1000)
            )
        ...
        if "process_memory_mb" in final_measurement:
            ...
            assert (
                memory_mb < 100
            ), f"Memory usage {memory_mb:.2f} MB exceeds 100MB stress limit"
...
class TestWebhookLoad:
    ...
    async def test_webhook_response_time(self):
        ...
        print(
            f"Webhook response times: avg={avg_time:.1f}ms, max={max_time:.1f}ms, p95={p95_time:.1f}ms"
        )
    ...
    async def test_webhook_burst_load(self):
        ...
        successful = sum(
            1 for result in results if isinstance(result, tuple) and result[0] == 202
        )
...
class TestAllocationPerformance:
    ...
    async def test_allocation_cycle_time(self, generator):
        ...
        allocation_engine = AllocationEngine(
            config, qbit_client, tracker_matcher, rollback_manager
        )
        ...
        assert (
            calc_time_1k < 2.0
        ), f"1000 torrent calculation took {calc_time_1k:.3f}s, too slow"
...
class TestEndToEndLoad:
    ...
    async def test_full_system_load(self):
        ...
        if "process_memory_mb" in final_measurement:
            print(
                f"  Final memory usage: {final_measurement['process_memory_mb']:.2f} MB"
            )
