...
from src.config import QguardarrConfig, GlobalSettings, QBittorrentSettings, TrackerConfig, RollbackSettings, CrossSeedSettings
...
class TestTorrentCache:
    ...
    def test_mark_for_update(self):
        ...
        index = cache.hash_to_index["hash1"]
        assert cache.needs_update[index] == False
        
        # Mark for update
        cache.mark_for_update("hash1")
        assert cache.needs_update[index] == True
...
class TestAllocationEngine:
    ...
    def mock_tracker_matcher(self):
        ...
        matcher.match_tracker.side_effect = lambda url: (
            "tracker1" if "tracker1.com" in url else
            "tracker2" if "tracker2.com" in url else
            "default"
        )
        matcher.get_tracker_config.side_effect = lambda tracker_id: Mock(
            id=tracker_id,
            max_upload_speed=5242880 if tracker_id == "tracker1" else
                              2097152 if tracker_id == "tracker2" else
                              1048576
        )
        return matcher
    ...
    @pytest.fixture
    def allocation_engine(self, config, mock_qbit_client, mock_tracker_matcher, mock_rollback_manager):
    ...
    def test_filter_torrents_for_rollout(self, allocation_engine):
        ...
        torrents = [
            TorrentInfo(hash=f"hash{i}", name=f"torrent{i}", state="uploading",
                       progress=1.0, dlspeed=0, upspeed=1000, priority=1,
                       num_seeds=5, num_leechs=2, ratio=1.5, size=1000000,
                       completed=1000000)
            for i in range(10)
        ]
    ...
    def test_calculate_limits_phase1_single_torrent(self, allocation_engine):
        ...
        torrents = [
            TorrentInfo(hash="hash1", name="torrent1", state="uploading",
                       progress=1.0, dlspeed=0, upspeed=1000, priority=1,
                       num_seeds=5, num_leechs=2, ratio=1.5, size=1000000,
                       completed=1000000, tracker="http://tracker1.com/announce")
        ]
    ...
    def test_calculate_limits_phase1_multiple_torrents(self, allocation_engine):
        ...
        torrents = [
            TorrentInfo(hash="hash1", name="torrent1", state="uploading",
                       ...
                       num_seeds=5, num_leechs=2, ratio=1.5, size=1000000,
                       completed=1000000, tracker="http://tracker1.com/announce"),
            TorrentInfo(hash="hash2", name="torrent2", state="uploading",
                       progress=1.0, dlspeed=0, upspeed=1000, priority=1,
                       num_seeds=5, num_leechs=2, ratio=1.5, size=1000000,
                       completed=1000000, tracker="http://tracker1.com/announce"),
            TorrentInfo(hash="hash3", name="torrent3", state="uploading",
                       progress=1.0, dlspeed=0, upspeed=1000, priority=1,
                       num_seeds=5, num_leechs=2, ratio=1.5, size=1000000,
                       completed=1000000, tracker="http://tracker1.com/announce")
        ]
    ...
    async def test_apply_differential_updates_no_changes(self, allocation_engine):
        ...
        success = allocation_engine.cache.add_torrent("hash1", "tracker1", 100.0, 1000000)
        ...
        assert current_limit == 1000000, f"Current limit is {current_limit}, expected 1000000"
    ...
    def test_get_tracker_stats(self, allocation_engine):
        ...
        allocation_engine.tracker_matcher.get_all_tracker_configs.return_value = [
            Mock(id="tracker1", name="Tracker 1", max_upload_speed=5242880, priority=10),
